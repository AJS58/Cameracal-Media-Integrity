...
function currentReportData(){
  return {
    version: "v5.7 Icons + Thumbnails Fix",
...
/* V5.7 thumbnail + corrected scan override */
(function(){
  const folder = document.getElementById('folderInput');
  const files = document.getElementById('fileInput');
  const chooseFolder = document.getElementById('chooseFolderVisible');
  ...
  const rawExt = new Set(['cr2','cr3','nef','arw','raf','rw2','orf','dng','pef','srw']);
  const imageExt = new Set(['jpg','jpeg','png','webp','tif','tiff','cr2','cr3','nef','arw','raf','rw2','orf','dng','pef','srw']);
  const previewExt = new Set(['jpg','jpeg','png','webp']);
  const videoExt = new Set(['mp4','mov','avi','mts','m2ts']);
  const PREVIEW_LIMIT = 20;

  function log(msg){ if(debug) debug.textContent = msg; console.log('[Cameracal V5.7]', msg); }
  ...
  function classify(file){
    ...
    const raw = rawExt.has(e);
    const previewable = previewExt.has(e);
    const image = imageExt.has(e);
    const video = videoExt.has(e);
    const supported = image || video;
    let status = 'good';
    let confidence = 'Good';
    let note = 'Extension and size check passed';
    ...
    if (file.size === 0 || !supported) {
      status = 'error';
      confidence = 'Error';
      note = file.size === 0 ? 'Zero-byte file' : 'Unsupported or unexpected file type';
    }
    ...
    return {
      ...
      raw, previewable, image, video, supported,
  ...
  function labelFor(score, errors, warnings){
    if (errors > 0 && score < 90) return {text:'High Risk', urgency:'WARNING'};
    if (score >= 98 && errors === 0 && warnings === 0) return {text:'Excellent', urgency:'SAFE'};
    if (score >= 90 && errors === 0) return {text:'Good', urgency:'SAFE'};
    if (score >= 75) return {text:'Caution', urgency:'CAUTION'};
    return {text:'High Risk', urgency:'WARNING'};
  }
  ...
  function updateMap(items){
    ...
    items.forEach(item => {
      const b = document.createElement('span');
      b.className = 'block';
      if(item.status === 'warning') b.classList.add('warn');
      if(item.status === 'error') b.classList.add('error');
      b.title = `${item.name} — ${item.confidence}`;
      map.appendChild(b);
    });
  }
  ...
  function makeRow(item, index){
    ...
    const canPreview = item.previewable && index < PREVIEW_LIMIT && item.file instanceof File;

    if (canPreview) {
      thumb.classList.add('preview');
      ...
      img.onerror = () => {
        ...
        thumb.classList.remove('preview');
        thumb.innerHTML = '<small>IMG</small>';
      };
      thumb.appendChild(img);
    } else {
      const label = item.raw ? 'RAW' : item.video ? 'VID' : item.image ? 'IMG' : item.extension.toUpperCase();
      thumb.innerHTML = `<small>${label}</small>`;
    }

    const middle = document.createElement('div');
    middle.innerHTML = `<strong>${item.name}</strong><small>${item.extension.toUpperCase()} • ${item.sizeLabel} • ${item.note}${item.previewable && index >= PREVIEW_LIMIT ? ' • thumbnail skipped after first 20 for performance' : ''}</small>`;

    const conf = document.createElement('div');
    conf.className = `confidence ${item.status === 'warning' || item.status === 'error' ? 'partial' : 'good'}`;
  ...
  async function scan(fileList){
    const selected = Array.from(fileList || []);
    if(!selected.length) return;
    log(`Scanning ${selected.length} selected file(s). JPEG thumbnails limited to first ${PREVIEW_LIMIT}.`);
    ...
    for(let i=0;i<selected.length;i++){
      const item = classify(selected[i]);
      items.push(item);
      if(item.status === 'good') good++;
      if(item.status === 'warning') warnings++;
      if(item.status === 'error') errors++;
      if(item.image || item.video) imageVideo++;
      if(i % 25 === 0) {
        log(`Checking ${i + 1} of ${selected.length}…`);
        await new Promise(r => setTimeout(r, 1));
      }
    }

    const score = scoreFromCounts(good, warnings, errors, selected.length);
    const result = labelFor(score, errors, warnings);

    window.__cameracalLastScan = {total:selected.length, good, warnings, errors, score, result:result.text, items};

    safeSet('cardStatus','Analysed');
    safeSet('healthScore', score + '%');
    safeSet('usableFiles', String(imageVideo));
    safeSet('issuesFlagged', String(warnings + errors));
    safeSet('urgency', result.urgency);
    safeSet('overviewScore', score + '%');

    const overview = document.getElementById('overviewText');
    if(overview) overview.innerHTML = `${selected.length} files checked locally. ${good} good, ${warnings} warning(s), ${errors} error(s).<span class="result-label">${result.text}</span>`;

    updateMap(items);

    if(list) {
      list.innerHTML = '';
      items.slice(0, 40).forEach((item, index) => list.appendChild(makeRow(item, index)));
      if(items.length > 40) {
        const more = document.createElement('p');
        more.className = 'muted';
        more.textContent = `Showing first 40 results from ${items.length}. Thumbnails are limited for performance.`;
        list.appendChild(more);
      }
    }

    log(`Scan complete: ${score}% ${result.text}. ${good} good, ${warnings} warnings, ${errors} errors.`);
  }

  if(folder) folder.onchange = e => scan(e.target.files);
  if(files) files.onchange = e => scan(e.target.files);
  if(chooseFolder && folder) chooseFolder.onclick = () => folder.click();
  if(chooseFiles && files) chooseFiles.onclick = () => files.click();
  if(testScan) testScan.onclick = () => {
    const fakeFiles = [
      {name:'IMG_0001.JPG', size:5200000, webkitRelativePath:'DCIM/100CANON/IMG_0001.JPG'},
      {name:'IMG_0002.CR3', size:34000000, webkitRelativePath:'DCIM/100CANON/IMG_0002.CR3'},
      {name:'BROKEN_FILE.JPG', size:0, webkitRelativePath:'DCIM/100CANON/BROKEN_FILE.JPG'}
  ...
  log('V5.7 icon and thumbnail fix loaded.');
})();
