...
function classifyFile(file, index){
  ...
  let note = "Header readable";

  if (isRaw) {
    confidence = "RAW metadata";
    confidenceClass = "raw";
    note = "Metadata-only check; preview skipped for performance";
  }
  if (suspicious) {
    confidence = "Partial";
    confidenceClass = "partial";
    note = file.size === 0 ? "Zero-byte file flagged" : "Needs manual review";
  }
  ...
  return {
    ...
    size: file.size,
    type: isRaw ? "RAW" : isJpeg ? "Image" : isVideo ? "Video" : "Other",
    isRaw,
    isJpeg,
    isVideo,
    isImage,
...
function updateProgress(done, total){
  ...
  progressText.textContent = done >= total ? "Scan complete" : `Scanning files ${done} of ${total}`;
...
function renderFileRows(reset = false){
  ...
  const next = analysedFiles.slice(renderedCount, renderedCount + (renderedCount === 0 ? INITIAL_RENDER : LOAD_MORE_STEP));
  next.forEach((item, i) => {
...
async function analyseFiles(fileListInput){
  ...
  document.getElementById('cardStatus').textContent = 'Scanning';
  document.getElementById('healthScore').textContent = '—';
  document.getElementById('usableFiles').textContent = '0';
  document.getElementById('issuesFlagged').textContent = '0';
  document.getElementById('urgency').textContent = 'SAFE';
  document.getElementById('overviewScore').textContent = '—%';
  document.getElementById('overviewText').textContent = 'Scanning selected files locally…';
  ...
  for (let start = 0; start < files.length; start += BATCH_SIZE) {
    ...
    // Yield to browser so the UI does not freeze on large cards.
  ...
  document.getElementById('usableFiles').textContent = imageCount;
  document.getElementById('issuesFlagged').textContent = issues;
  document.getElementById('urgency').textContent = urgency;
  document.getElementById('overviewScore').textContent = `${score}%`;
  document.getElementById('overviewText').textContent = `${files.length} files checked locally. ${imageCount} image/video files identified.`;

  renderMap(issues > 10 ? 11 : 17, issues > 0 ? 8 : 13);
  renderFileRows(true);
}

folderInput.addEventListener('change', e => analyseFiles(e.target.files));
fileInput.addEventListener('change', e => analyseFiles(e.target.files));
loadMoreBtn.addEventListener('click', () => renderFileRows(false));
...
Performance mode:
Large cards are processed in batches. RAW files are treated as metadata-only in the browser to avoid freezing.`;
  downloadBlob("cameracal-card-health-v5-2-diagnostic-log.txt", content, "text/plain");
}

function generateHtmlReport(){
  const d = currentReportData();
  const rows = analysedFiles.slice(0, 40).map(item => `
    <tr>
      <td>${item.name}</td>
      <td>${item.ext}</td>
...
<div class="report">
  ...
  <div class="grid">
    ...
    <div class="card"><span>Files checked</span><strong>${d.totals.total}</strong></div>
    <div class="card"><span>Image/video files</span><strong>${d.totals.images}</strong></div>
  ...
  <h2>File summary</h2>
  <p>RAW files: ${d.totals.raw} · JPEG/standard images: ${d.totals.jpeg} · Video files: ${d.totals.video}</p>

  <h2>Sample results</h2>
  <table>
    <thead><tr><th>File</th><th>Type</th><th>Size</th><th>Confidence</th></tr></thead>
    <tbody>${rows || '<tr><td colspan="4">No files analysed yet.</td></tr>'}</tbody>
  </table>
  ...
  downloadBlob("cameracal-card-health-v5-2-interactive-report.html", html, "text/html");
}

function generatePdfReport(){
  const d = currentReportData();
  const rows = analysedFiles.slice(0, 25).map(item => `
    <tr><td>${item.name}</td><td>${item.ext}</td><td>${formatBytes(item.size)}</td><td>${item.confidence}</td></tr>
  `).join('');
...
h1{font-size:25px;margin-bottom:4px}.sub{color:#64748b;margin-bottom:18px}
...
.warning{border:1px solid #ffc6c6;background:#fff4f4;color:#8c2626;border-radius:10px;padding:12px;margin-top:16px}
...
<h2>Sample results</h2>
<table><thead><tr><th>File</th><th>Type</th><th>Size</th><th>Confidence</th></tr></thead><tbody>${rows || '<tr><td colspan="4">No files analysed yet.</td></tr>'}</tbody></table>
<div class="warning"><strong>Important recovery rule:</strong><br>Stop using the card immediately. Do not format it or save recovered files back to the same card.</div>
<div class="privacy"><strong>Privacy first:</strong><br>This browser version analyses selected files locally. No images are uploaded to a server.</div>
<div class="footer">This report is a browser-based media health assessment. True deleted-sector recovery requires a desktop recovery engine.</div>
<script>window.onload = () => window.print();</script>
  ...
  if (!reportWindow) {
    alert("The PDF report window was blocked by the browser. Please allow popups for this site and try again.");
    return;
  }
...
document.getElementById('exportReport').addEventListener('click', (event) => {
  event.stopPropagation();
  exportMenu.classList.toggle('hidden');
});
document.getElementById('exportHtml').addEventListener('click', generateHtmlReport);
document.getElementById('exportPdf').addEventListener('click', generatePdfReport);
document.getElementById('exportTxt').addEventListener('click', generateTextReport);

if (document.getElementById('htmlReport')) document.getElementById('htmlReport').addEventListener('click', generateHtmlReport);
if (document.getElementById('pdfReport')) document.getElementById('pdfReport').addEventListener('click', generatePdfReport);
if (document.getElementById('txtReport')) document.getElementById('txtReport').addEventListener('click', generateTextReport);
...
document.getElementById('checkUpdatesBtn').addEventListener('click', () => {
  alert('This no-cache test build does not use automatic PWA updates. Upload the newest files to GitHub Pages to update the app.');
});
